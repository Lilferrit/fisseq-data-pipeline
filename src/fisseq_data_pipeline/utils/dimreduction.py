...
import/JIT-compilation cost should only be paid by runs that actually set
``run_umap=true``. ``scikit-learn`` is imported eagerly at module top since
it's already an unconditional dependency of this pipeline.
"""
...
def _select_fittable_features(
    ...
    they are dropped here, with a warning naming what was dropped. This
    handles the all-null case only -- a feature that is null for only some
    rows still reaches PCA/UMAP unmodified and will raise there.
...
def compute_pca(
    df: pl.DataFrame, label_column: str, n_components: int
    ...
    # random_state is defense-in-depth only: sklearn only consults it for the
    # randomized-SVD solver path, which this pipeline's matrix sizes are
    # unlikely to trigger (the "auto"/"full" solver is otherwise
    # deterministic) -- not exposed as a user-facing param.
