...
Hydra entry point backing the Nextflow process ``BATCH_CORRECT_FIT``: fits
per-(variant, batch) statistics and per-variant centroids across all batches via
:class:`BatchCorrector`. The transform entry point (``BATCH_CORRECT_TRANSFORM``),
which rescales each batch's cells to its variant's centroid and finally to the
wildtype centroid using a fitted :class:`BatchCorrector`, lives in
:mod:`.batchcorrecttransform`.
"""
