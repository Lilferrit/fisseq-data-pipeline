...
NORMALIZE        (per batch)   ← z-score fit on WT control cells
      ...
      │
      ├──► WTVVARIANTPOOL_BATCHWISE (per batch; skipped unless params.run_wtvvariantpool
      │           = true, default FALSE; pools non-wildtype cells whose classified
      │           variant class is in wtvvariantpool_variant_classes, one binary
...
(for on-disk inspection) and, downstream, feeds those same per-batch outputs
into `ANOVA`/`BATCHVSBATCH`/`BATCH_CORRECT_FIT` as real per-channel `path`
inputs (collected via `workflows/fisseq.nf`'s `perChannelSignal`), not a
re-globbed directory string — see gotcha 6 below.
...
this cell-staging mechanism at all — it consumes each member batch's own
`AGGREGATE_FEATURE_TYPE_BATCHWISE`/`COMBINE_BLOCKLISTS_BATCHWISE` outputs
directly (real Nextflow `path` inputs, scoped per channel; member batches
resolved from `resolvedBatchConfigs` in Groovy for logging only, only those
with `run_feature_selection` enabled), since it has no cell-level work to
do — see
...
`WTVVARIANTPOOL_BATCHWISE` (per batch) is likewise a single, unaliased process
(`modules/local/wtvvariantpool_batchwise.nf`, wrapping
`python -m fisseq_data_pipeline.wtvvariantpool`). It pools cells whose
`label_column` value classifies (via `utils/variant.py:classify_variant`)
into one of `params.wtvvariantpool_variant_classes` (default
`["Synonymous"]`), excluding wildtype-labeled rows up front by construction
(not via `classify_variant`'s own `"WT"` category, since `wt_label` is
configurable). After dropping wildtype barcodes below
`params.wtvvariantpool_min_cells_per_barcode`, it trains one binary XGBoost
classifier per surviving barcode vs. the pool, optionally downsampling the
pool via `params.wtvvariantpool_downsample_variant_pool` (`null`/`false`
disabled, `true` matches the largest surviving barcode's cell count, an int
is an exact target). Gated per batch by `params.run_wtvvariantpool`,
...
│   ├── wtnullaggregate.py         # WT-null bootstrap replicate entry point (imports aggregate.py)
│   ├── wtnullblocklist.py         # WT-null Tukey-fence blocklist entry point
│   ├── passthroughblocklist.py    # Trivial all-ok blocklist entry point (non-WT-null feature types)
...
│   ├── wtvvariantpool.py          # Wildtype-barcode-vs-variant-pool XGBoost classifier + entry point
...
│   ├── wtvwt_batchwise.nf
│   ├── wtvvariantpool_batchwise.nf
│   ├── check_barcodes.nf
│   ├── barcode_blocklist.nf
│   ├── aggregate_feature_type.nf
│   ├── wt_null_aggregate.nf
│   ├── wt_null_blocklist.nf
│   ├── passthrough_blocklist.nf
...
├── tests/
│   ├── unit/                      # fast, synthetic data (incl. utils/ subpackage)
...
**`BaseAggregator`** (`aggregate.py`) — abstract base for 10 concrete aggregation strategies (mean, median, MAD, std, KS, signedKS, QQ, AUROC, KSnegLogP, AUROCnegLogP). There is no multi-aggregator wrapper; combining feature types happens in Nextflow — `aggregate.feature_type_main` runs once per `params.feature_select_types` entry, and `features.main` (the final feature-selection stage) joins the per-feature-type outputs on the label column. Each aggregator also declares its own WT-null eligibility via `null_statistic_transform` (raises `NotImplementedError` by default; concretely overridden by every aggregator except `MAD`/`KSnegLogP`/`AUROCnegLogP`) and computes its WT-null comparison via `null_comparison_statistic` — a one-sample diff-of-halves default on `BaseAggregator` for aggregators with no reference pool (mean/median/std), overridden on `ReferenceBasedAggregator` with a two-sample relabel-and-compare path for KS/signedKS/QQ/AUROC. See `is_null_eligible`/`null_eligible_aggregator_names` and `wtnullaggregate.py`.

**`utils/xgbparams.py`** — shared XGBoost infrastructure imported by `ovwt.py`, `ovwtcellscores.py`, `batchvsbatch.py`, `wtvwt.py`, and `wtvvariantpool.py`. Contains: `XGBoostParams` and `XGBoostConfig` dataclasses; `get_feature_cols` (CellProfiler column detection); `get_dmatrix` (binary DMatrix builder); `get_dmatrix_multiclass` (multiclass DMatrix with sorted integer encoding); `split_indices_stratified` (80/10/10 stratified split on any label array); `resolve_feature_importance` (maps a trained booster's gain-based `get_score()` output, keyed by internal `f<i>` index since `get_dmatrix`/`get_dmatrix_multiclass` never pass `feature_names`, back onto real feature names via a `feature_cols` list in DMatrix-construction order). Do not add XGBoost-specific infrastructure to individual modules — put it here. Each of `ovwt.py`, `batchvsbatch.py`, `wtvwt.py`, and `wtvvariantpool.py` calls `resolve_feature_importance` per trained model and assembles its own `feature_importance.parquet` (one row per model, columns named by real feature) using its own group-column convention (`cfg.label_column` for `ovwt.py`/`batchvsbatch.py`, `barcode` for `wtvvariantpool.py`, `barcode_a`/`barcode_b` for `wtvwt.py`) — the per-module dict assembly is intentionally not centralized further, matching each module's own results schema. `wtvwt.py` reuses `get_dmatrix`/`get_feature_cols`/`split_indices_stratified` directly (calling `get_dmatrix` with `barcode_column`/one barcode of a pair in place of `label_column`/`wt_label` — the function is already fully generic), rather than adding a barcode-specific DMatrix helper. `wtvvariantpool.py` reuses the same trick: it writes a reserved sentinel value into `barcode_column` for pooled non-wildtype rows, standing in for "the other barcode" against each wildtype barcode in turn.

**`utils/filtering.py`** — shared row/group filtering and downsampling helpers imported by `wtvwt.py`, `ovwt.py`, `batchvsbatch.py`, and `wtvvariantpool.py`. Contains: `_exclude_blocked_features` (drop feature columns per a block-list parquet); `drop_small_groups` (drop groups below a minimum row count, no exemptions); `downsample_group_to_target` (downsample one group to a target count — an explicit `n`, or by default the largest per-value row count among every *other* group). Each consuming module keeps a thin, same-named wrapper where its own call sites depend on module-specific parameter names — `wtvwt.py`'s `filter_min_cells_per_barcode` wraps `drop_small_groups`, `ovwt.py`'s `downsample_wildtype` wraps `downsample_group_to_target`. Do not re-duplicate this logic in a new module — import it from here.
...
**Nextflow synchronization pattern** (`workflows/fisseq.nf`): `ANOVA`/`BATCHVSBATCH`/`OVWT_GLOBAL`/`BATCH_CORRECT_FIT` all wait per active global channel, via `channel_qc_signal_ch`/`channel_norm_signal_ch` (each built by `perChannelSignal` from `STAGE_CHANNEL_QC`/`STAGE_CHANNEL_NORM`'s output collected with `.groupTuple()`, keyed by channel — see "Global channels" above and gotcha 6 below) rather than one flattened signal; each now carries that channel's real, per-batch `path` list (`(chan, [path, ...])`), not a directory string to re-glob. `GLOBAL_FEATURE_SELECT` uses neither of those two signals — it's driven directly by `agg_ch`/`combined_bl_ch` (`AGGREGATE_FEATURE_TYPE_BATCHWISE`'s/`COMBINE_BLOCKLISTS_BATCHWISE`'s own outputs), scoped per channel and `.groupTuple()`'d the same way, since it only ever needs `feature_select_batchwise/` artifacts, never staged cells. `ANOVA` is a single parameterized process (`modules/local/anova.nf`) invoked twice via Nextflow's `include { X as Y }` aliasing (a process cannot be called twice under its own name in one workflow), each alias additionally invoked once per active global channel and threading `channel` through its output tuple so downstream consumers can join back to the right channel; `BATCHVSBATCH` (`modules/local/batchvsbatch.nf`) is likewise aliased twice, each alias additionally invoked once per active global channel — `BATCHVSBATCH_PRE` waits on `channel_qc_signal_ch` and receives that channel's staged `qc_filter_cells` files as a real `path` input with `use_parent_name=false` (every `STAGE_CHANNEL_CELLS` output is flattened to `<batch_stem>.parquet`, so parent-dir naming is no longer needed), unfiltered; `BATCHVSBATCH_POST` waits on `channel_norm_signal_ch` joined with `anova_blocklist_ch` (`ANOVA_BLOCKLIST`'s output, now one-per-channel) and receives that channel's staged `normalization_cells` files the same way, filtered. `ANOVA_NORMALIZED` waits on `channel_norm_signal_ch` and `ANOVA_BATCH_CORRECTED` waits on `channel_bc_signal_ch` (built from `BATCH_CORRECT_TRANSFORM`'s own output the same `perChannelSignal` way) — both ANOVA calls run once per active global channel, scoped to only that channel's batches; with no active channels, neither runs at all. `ANOVA_NORMALIZED`'s call sits earlier in `workflows/fisseq.nf` than `ANOVA_BATCH_CORRECTED`'s (right after `channel_norm_signal_ch` is computed, not at the bottom) so its output channel can feed `ANOVA_BLOCKLIST`, which in turn is `.join()`ed (per-channel, not a broadcast) onto `OVWT_GLOBAL` and `BATCHVSBATCH_POST` — `.join()` is safe here specifically because both sides are already collapsed to exactly one item per channel; `BATCH_CORRECT_TRANSFORM` instead uses `.combine(fit_out, by: 0)` for `BATCH_CORRECT_FIT`'s output, since the batch side genuinely fans out N batches per channel (not one-per-channel), the many-to-one shape `.join()` must never be used for. `OVWT_BATCHWISE` is likewise a single parameterized process (`modules/local/ovwt_batchwise.nf`) aliased, in `FisseqPipeline`, into `OVWT_BATCHWISE_UNFILTERED` (no dependency on `ANOVA_BLOCKLIST` or `BARCODE_BLOCKLIST`; optional, gated by `params.run_ovwt`, default `true`) and `OVWT_BATCHWISE_BARCODE_FILTERED` (depends on that batch's `BARCODE_BLOCKLIST` output; optional, gated by `params.run_barcode_filtered_ovwt`, default `true`); a third alias, `OVWT_BATCHWISE_FEATURE_FILTERED` (gated by the now-removed `params.run_feature_filtered_ovwt`), used to exist but was removed once `ANOVA_BLOCKLIST` became per-channel (see "Global channels" above); the varying `feature_block_list_file`/`barcode_block_list_file` values are each passed as a `val` (not a Nextflow-staged `path`) so a Groovy `null` can flow straight through to `python -m fisseq_data_pipeline.ovwt`'s `feature_block_list_file=null`/`barcode_block_list_file=null` CLI args for calls that don't use them, the same `null`-passthrough trick `qc_filter.nf` uses for `qc_downsample_amounts`/`qc_n_variants`. All varying bits (cells path list, `use_parent_name`, the two block-list vals, `publishDir` subpath) are passed in as process input values, not hardcoded per-call. Since `BARCODE_BLOCKLIST` runs per batch (unlike the per-channel `ANOVA_BLOCKLIST`), `OVWT_BATCHWISE_BARCODE_FILTERED`'s input channel uses `norm_ch.join(barcode_blocklist_ch)` (both already keyed one-per-batch_stem) rather than `.combine()` (which is reserved for broadcasting a single value across every element of another channel). `OVWT_BATCHWISE`'s output tuple carries both `test_index.parquet` and `train_index.parquet` (the underlying `ovwt.py:main` always writes both, plus `val_index.parquet`, when `save_splits=True`; only the first two are wired into Nextflow).

**Feature-selection pipeline** (`workflows/fisseq.nf`) has two independent parts. The BATCHWISE branch follows the same aliasing pattern as `ANOVA`/`BATCHVSBATCH`, applied to 6 processes (`AGGREGATE_FEATURE_TYPE`, `WT_NULL_AGGREGATE`, `WT_NULL_BLOCKLIST`, `PASSTHROUGH_BLOCKLIST`, `COMBINE_BLOCKLISTS`, `FINALIZE_FEATURE_SELECT`), each aliased once as `*_BATCHWISE` (no `*_GLOBAL` alias exists anymore — see below). `AGGREGATE_FEATURE_TYPE_BATCHWISE` runs once per (batch, feature type) in `params.feature_select_types`, unconditionally; its output (`agg_ch`) then splits into two reproducibility-gate branches based on `params.feature_select_wt_null_types` (validated at workflow start: must be a subset of `feature_select_types`, and none may be `MAD` — the only registered aggregator that is not WT-null-eligible; `mean`/`median`/`std` are WT-null-eligible via a one-sample diff-of-halves comparison and may be opted in, though the default list only includes `KS`/`QQ`/`AUROC`). For feature types in that subset: `wt_null_types_ch`/`bootstrap_ch` (`1..params.feature_select_wt_null_bootstraps`) are crossed via `.combine()` to fan out `WT_NULL_AGGREGATE_BATCHWISE`, and `WT_NULL_BLOCKLIST_BATCHWISE`'s `groupTuple(by: [batch_stem, feature_type])` — gathering all `params.feature_select_wt_null_bootstraps` replicates for one feature type before applying an upper Tukey fence over the batch's per-feature null-mean distribution — is the pipeline's only cross-bootstrap synchronization point. For every other configured feature type: `agg_ch` is filtered directly into `PASSTHROUGH_BLOCKLIST_BATCHWISE` (no bootstrap fan-out, no cell-level recompute). Both branches' outputs are merged with `.mix()` before `COMBINE_BLOCKLISTS_BATCHWISE`. `params.run_feature_selection` (default `true`, per-batch overridable) gates this whole branch per batch, via `norm_ch_feature_selected`'s `.filter()`.

`GLOBAL_FEATURE_SELECT` (`globalfeatureselect.py`, `modules/local/global_feature_select.nf`) is a separate, single, non-aliased process — not part of the 6-process aliasing pattern above, and no bootstrap machinery of its own. It runs once per active channel in `params.global_channels` (default `null` — none run at all), also gated on the pipeline-wide `params.run_feature_selection`. Per channel, batch membership (only batches with THEIR OWN `run_feature_selection` enabled) is still resolved directly from `resolvedBatchConfigs` in Groovy (`batchesByChannel`, built with `.collectEntries`/`.findAll`) for diagnostic logging (a `log.warn` when a channel has no member batches) — but the actual data wiring is driven by `agg_ch`/`combined_bl_ch` (`AGGREGATE_FEATURE_TYPE_BATCHWISE`'s/`COMBINE_BLOCKLISTS_BATCHWISE`'s real outputs), `.filter()`ed to each channel's member batches and `.groupTuple()`'d into parallel `(agg_batch_stems, agg_files)`/`(bl_batch_stems, blocklist_files)` lists, `.join()`ed per channel (both sides already one row per channel) and passed as real `path` inputs — not re-derived from a plain `val(batch_stems)` + `val(pipeline_dir)` Python-side glob (see gotcha 6). Since every batch's aggregate/blocklist files share the same basename across batches, the process stages them via `stageAs` auto-numbering (`agg_input_1.parquet`, `agg_input_2.parquet`, ...; `bl_input_1.parquet`, ...) so the same-named files from different batches don't collide; the parallel `agg_batch_stems`/`bl_batch_stems` `val` lists (built from the same source list, so ordering matches the staged numbering) tell the Python side which staged file belongs to which batch. `globalfeatureselect.py`'s `main()` groups each batch's own aggregate files, joins per-feature-type files, normalizes to that batch's own synonymous baseline, takes the cross-batch median, combines blocklists by agreement threshold (`params.global_feature_select_min_batches_ok`), and runs `pyc_feature_select` — a batch present on only one side of `agg_batch_stems`/`bl_batch_stems` (e.g. because one of its upstream tasks failed under `errorStrategy 'ignore'`) is excluded from that side with a warning logged, not treated as a hard failure of the whole channel. `OVWT_GLOBAL`, both aliased `BATCHVSBATCH` calls (`_PRE`/`_POST`), `ANOVA` (`_NORMALIZED`/`_BATCH_CORRECTED`), and the batch-correction branch (`BATCH_CORRECT_FIT`/`BATCH_CORRECT_TRANSFORM`) all likewise run once per active channel, independently of `GLOBAL_FEATURE_SELECT` and of each other (e.g. run global processes separately per experiment/site/cohort, or not at all).
...
| `python -m fisseq_data_pipeline.aggregatefeaturetype` | Lean per-feature-type full aggregation |
| `python -m fisseq_data_pipeline.wtnullaggregate` | One WT-null bootstrap replicate: split the control pool, compute the aggregator between the two halves per feature |
| `python -m fisseq_data_pipeline.wtnullblocklist` | Gather WT-null bootstrap replicates, apply an upper Tukey fence, blocklist for one feature type |
| `python -m fisseq_data_pipeline.passthroughblocklist` | Trivial all-ok blocklist for one feature type (no WT-null check) |
...
| `python -m fisseq_data_pipeline.wtvvariantpool` | Wildtype-barcode-vs-variant-pool: one binary XGBoost classifier per wildtype barcode vs. a pooled non-wildtype variant set |
...
<pipeline_dir>/
  ...
  ovwt_batchwise/<batch>/       # unfiltered — full feature set
    results.parquet
    models.pkl
    feature_importance.parquet  # one row per variant, one column per feature that
                                 # appeared in at least one variant's splits, plus
                                 # cfg.label_column (gain-based, from xgb.Booster.get_score)
  ...
  wtvwt_batchwise/<batch>/      # wildtype cells only; optional: params.run_wtvwt
    ...
    feature_importance.parquet  # one row per pair, one column per feature that appeared
                                 # in at least one pair's splits, plus barcode_a/barcode_b
  wtvvariantpool_batchwise/<batch>/  # optional: params.run_wtvvariantpool (default false)
    results.parquet             # one row per surviving wildtype barcode; columns: barcode,
                                 # train/val/test_auroc, train/val/test_accuracy, n_cells_barcode, n_cells_pool
    models.pkl                  # dict[barcode -> xgb.Booster]
    feature_importance.parquet  # one row per barcode, one column per feature that
                                 # appeared in at least one barcode's splits, plus barcode
  ...
  global/<channel>/               # one subtree per channel in params.global_channels
    ...
    batchvsbatch/
      ...
      pre/feature_importance.parquet   # one row per variant, one column per feature that
                                        # appeared in at least one variant's splits, plus cfg.label_column
      ...
      post/feature_importance.parquet
    anova/
...
6. **Global Nextflow processes receive their scoped cells/artifacts as real `path` inputs, not a re-globbed directory string.** An earlier version of this pipeline had `BATCHVSBATCH`, `OVWT_GLOBAL`, `ANOVA`, `BATCH_CORRECT_FIT`, and `GLOBAL_FEATURE_SELECT` each take a directory-glob `val` (e.g. `"${dir}/*.parquet"`) and re-read the matching files from disk inside the process script. That broke `-resume` cache invalidation: Nextflow only hashes a `val`'s string value, not the file *set* it resolves to, so editing/adding/removing an underlying file without the glob string itself changing left the task incorrectly cached. Fixed by keeping the individual `path`s Nextflow already produces upstream, instead of discarding them and rebuilding a directory string: `STAGE_CHANNEL_CELLS` (`modules/local/stage_channel.nf`) republishes each channel's member batches' `QC_FILTER`/`NORMALIZE` output into `global/<channel>/{qc_filter_cells,normalization_cells}/` (flattened to `<batch_stem>.parquet`, still useful for on-disk inspection), and `workflows/fisseq.nf`'s `perChannelSignal` collects those same `path`s per channel via `.groupTuple()` (rather than collapsing them to a directory string) to feed `BATCHVSBATCH_PRE`/`BATCH_CORRECT_FIT` (that channel's `qc_filter_cells` files) and `BATCHVSBATCH_POST`/`OVWT_GLOBAL`/`ANOVA_NORMALIZED` (that channel's `normalization_cells` files) as a real `path(cells_files)` process input — each module's script then globs the *staged* files locally (`"input_file=./*.parquet"`) purely to satisfy the underlying Python CLI's glob-accepting `input_file=` argument, which is a cosmetic detail, not the cache-relevant part. `GLOBAL_FEATURE_SELECT` follows the same fix against a different pair of upstream outputs: it consumes `AGGREGATE_FEATURE_TYPE_BATCHWISE`'s/`COMBINE_BLOCKLISTS_BATCHWISE`'s real per-batch outputs (`agg_ch`/`combined_bl_ch`), scoped per channel and grouped via `.groupTuple()`, rather than re-deriving `feature_select_batchwise/<batch>/{aggregates,blocklist.parquet}` paths from a `val(pipeline_dir)` + `val(batch_stems)` Python-side glob. Since every batch's aggregate/blocklist files share the same basename across batches, `modules/local/global_feature_select.nf` disambiguates them via `stageAs` auto-numbering (`agg_input_1.parquet`, `agg_input_2.parquet`, ...), paired with parallel `agg_batch_stems`/`bl_batch_stems` `val` lists (built from the same source list, so ordering matches) that `globalfeatureselect.py`'s `main()` uses to group each staged file back to its owning batch — see that module's docstring. It still has no dependency on `STAGE_CHANNEL_CELLS`/`normalization_cells/` at all -- that mechanism is unrelated to this one.

7. **When `output_root` is set, it takes priority over `output_dir` in the underlying Hydra CLIs** (`aggregate.py`, `featureselect.py`) — the output file lands at `{output_root}.{stem}.parquet` regardless of `output_dir`, matching pre-existing behavior in `aggregate.py:main`/`featureselect.py:main`. `aggregate_feature_type.nf` and `finalize_feature_select.nf` both pass `output_root` and therefore use `output_dir=.` (not a subdirectory) plus a glob-based `mv` to rename the result to the process's declared output filename. `finalize_feature_select.nf` additionally does `mkdir -p ft && mv ${feature_type_files} ft/` to isolate the multi-file `feature_type_files` input from the co-staged `block_list_file` before globbing. If you change output naming in `featureselect.py`/`aggregate.py`, update these workarounds. `globalfeatureselect.py`/`global_feature_select.nf`, `wtnullblocklist.py`/`wt_null_blocklist.nf`, and `passthroughblocklist.py`/`passthrough_blocklist.nf` do not follow this pattern at all — no `output_root`; each always writes a fixed filename (`aggregate.parquet`/`blocklist.parquet`, `blocklist.parquet`, `blocklist.parquet` respectively), and the latter two's Nextflow wrappers `mv` it to `${feature_type}.parquet` since multiple feature types are gathered into the same downstream glob (matching the old `blocklist.py`/`combineblocklists.py` convention). `wtnullaggregate.py`/`wt_null_aggregate.nf` is a partial exception: no `output_root` either (always writes a fixed `wt_null.parquet`), but its Nextflow wrapper still `mv`s the result, to `bootstrap_${bootstrap_idx}.parquet` — needed so `WT_NULL_BLOCKLIST`'s glob doesn't collide across bootstrap replicates within the same feature type.
...
13. **Any batch YAML in `configs/` can override most `nextflow.config` params for just that batch** — resolved once per batch, in Groovy, by `lib/BatchParams.groovy`'s `resolve()` (see `docs/configuration.md`'s "Per-batch parameter overrides" and "Global channels" sections for the full mechanism; unlike most of `docs/`, that page is current and was written alongside this code). Every override is logged via `log.info` at workflow-construction time — never buried in a process script. Four things to keep in mind:
    ...
    - Not every param has a per-batch meaning. `--global_channels`, `--batchvsbatch_min_cells`, `--batchvsbatch_min_batches`, `--anova_blocklist_pvalue_threshold`, `--feature_select_types`, `--feature_select_wt_null_types`, `--feature_select_wt_null_bootstraps`, and `--global_feature_select_min_batches_ok` are consumed only by processes that run once across *all* (or, for most of these, all of one channel's) batches (`BATCHVSBATCH`, `OVWT_GLOBAL`, `ANOVA_BLOCKLIST`, `GLOBAL_FEATURE_SELECT`) or that determine shared fan-out cardinality/DAG shape, so they're pipeline-wide-only — a batch YAML that tries to set one of these gets a clear rejection error (distinct wording from the plain-unrecognized-key error), not a silent no-op.
