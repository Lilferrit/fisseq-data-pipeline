...
| `aggregate_feature_chunk_size` | `64` | Feature columns `AGGREGATE_FEATURE_TYPE` / `AGGREGATE_HALF` evaluate per Polars query. A memory dial: peak memory scales with `chunk_size × n_variant_labels` (× the control pool, for the reference-based aggregators), while runtime is essentially flat in it. Halve it if a task is OOM-killed (exit 137); raise it for runs using only `mean`/`median`/`std`/`MAD`. See [Feature chunking](cli/aggregate.md#feature-chunking). |
