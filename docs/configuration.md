...
| `--run_ovwt_lobo` | `false` | `FisseqPipeline` only: run `OVWTLOBO_BATCHWISE` (per-batch, leave-one-barcode-out OvWT generalization test) for that batch. Independent of `--run_ovwt`/`--run_wtvwt`/`--run_wtvvariantpool` and every other gate. Default `false`, like `--run_wtvvariantpool` (LOBO trains many more models per batch than a single `OVWT_BATCHWISE` pass). |
...
### Leave-one-barcode-out OvWT generalization (`OVWTLOBO_BATCHWISE`)
...
| `--feature_select_wt_null_types` | `["KS", "QQ", "AUROC"]` | Strict subset of `--feature_select_types` that uses the WT-null bootstrap reproducibility gate (`WT_NULL_AGGREGATE`/`WT_NULL_BLOCKLIST`). `mean`/`median`/`std` may also be opted in (they use a one-sample diff-of-halves comparison rather than a reference-pool comparison, but are not included by default); `MAD` is not eligible (its own sampling distribution is lumpier at small n / low-cardinality features, and its WT-null reproducibility hasn't been validated). Every other configured feature type is auto-approved by `PASSTHROUGH_BLOCKLIST` instead (no reproducibility computation). Validated at workflow start: an entry not also in `--feature_select_types`, or `MAD`, fails fast with a clear error. |
...
`--run_barcode_filtered_ovwt`, `--run_wtvwt`, `--run_wtvvariantpool`,
`--run_ovwt_lobo`, and the
...
`--wtvvariantpool_downsample_variant_pool`, `--ovwt_lobo_min_cells_holdout`,
and `--ovwt_lobo_min_barcodes_per_variant`. Each of these gates only a
