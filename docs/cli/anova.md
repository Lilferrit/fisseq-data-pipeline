...
column's per-batch-group sum, sum of squares, and count are computed in a
single pass and used to derive an F-statistic via the standard one-way ANOVA
sum-of-squares decomposition. A closed-form p-value is computed from the
F-distribution's survival function (`scipy.stats.f.sf`), with no permutation
test involved.
...
| `feature_batch_size` | `null` | Optional int: number of feature columns collected per query, bounds peak memory when there are many features. `null` collects all features in one query. |
