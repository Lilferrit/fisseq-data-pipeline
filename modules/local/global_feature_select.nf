...
// same-named-file staging collision to design around. The channel identifier
// is named "chan" below -- "channel" is a reserved Nextflow binding
// (lowercase alias for the Channel class), see AGENTS.md.
process GLOBAL_FEATURE_SELECT {
    ...
    input:
    tuple val(chan), val(batch_stems), val(pipeline_dir), val(publish_subdir), val(min_batches_ok)
