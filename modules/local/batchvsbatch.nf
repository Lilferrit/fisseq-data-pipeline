...
// post-normalization (filtered against the ANOVA_BLOCKLIST output).
...
process BATCHVSBATCH {
    ...
    input:
    tuple val(input_dir), val(cells_glob), val(use_parent_name), val(publish_subdir), val(block_list_file)
